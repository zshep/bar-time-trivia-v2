...
export function registerSocketHandlers(io, socket) {
  ...
  socket.on(
    "create-session",
    ({ sessionCode, hostId, hostName, gameName, gameId }) => {
      console.log(
        `Session created: ${sessionCode} by host ${hostName} with id: ${hostId} for game ${gameName}`,
      );
  ...
  socket.on("player-answer", (payload, ack) => {
    ...
    const { sessionCode, questionId, choiceIndex, choiceText, choice } =
      payload;
    ...
    console.log(
      `player ${socket.id} has submitted ${choiceText} for question ${questionId}`,
    );
  ...
  socket.on("reconnect-player", ({ sessionCode, userId }) => {
    console.log(
      `reconnect-player from ${socket.id} for userId: ${userId} to session: ${sessionCode}`,
    );
  ...
  socket.on("results-finalized", ({ finalRoundData }) => {
    ...
    if (!Number.isInteger(roundIndex) || roundIndex < 0) {
      console.warn(
        "results-finalized: invalid roundindex",
        finalRoundData?.roundIndex,
      );
      return;
    }

    const scores = Array.isArray(finalRoundData?.scores)
      ? finalRoundData.scores
      : [];
  ...
  function upsertRoundTotals(session, roundIndex, entries) {
    ...
    if (!session.questionsAnsweredByPlayer)
      session.questionsAnsweredByPlayer = {};
    if (!session.questionsCorrectByPlayer)
      session.questionsCorrectByPlayer = {};

    //idempotency
    if (session.finalizedRounds.has(roundIndex)) {
      console.log(
        "upsertRoundTotals: round already finalized, recomputing only. roundIndex=",
        roundIndex,
      );
    ...
    for (const {
      ...
      session.currentPlayerScores[pid].byRound[roundIndex] =
        Number(roundScore) || 0;
  ...
  function recomputeTotals(session) {
    ...
    for (const [pid, rec] of Object.entries(session.currentPlayerScores)) {
      console.log(
        ...
        pid,
        "byRound=",
        JSON.stringify(rec.byRound),
      );

      const sum = Object.values(rec.byRound || {}).reduce(
        (a, b) => a + (Number(b) || 0),
        0,
      );
      rec.total = sum;
  ...
  socket.on("next-round", ({ sessionCode }) => {
    ...
    console.log(
      `host is starting round index ${session.currentRound} of ${totalRounds}`,
    );
  ...
  socket.on("end-game", async ({ sessionCode }) => {
    ...
    if (session) {
      ...
      for (const [pid, rec] of Object.entries(
        session.currentPlayerScores || {},
      )) {
        const userMatch = session.players.find((p) => p.userId === pid);
